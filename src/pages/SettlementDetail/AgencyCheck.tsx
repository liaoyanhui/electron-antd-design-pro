 ...
 * @Description:
 * @Author: 尚夏
 * @Date: 2021-09-13 09:32:25
 * @LastEditTime: 2022-01-13 10:19:32
 * @FilePath: /mining-admin-desktop/src/pages/SettlementDetail/AgencyCheck.tsx
 */
...
const AgencyCheck: React.FC = () => {
  ...
  const formatArr = (arr: any[]) => {
    const newArr: any[] = [
      ['用户', '邀请了', '存储(T)', '立刻释放', '锁仓', '历史释放', '总锁仓', '本次结算'],
    ];
  ...
  useEffect(() => {
    ...
    if (settlementNo) {
      FilecoinService.getFilecoinReferrerSettlement(settlementNo, {
        filterBy: 'all',
      }).then((res: any) => {
        setData(res);

        // 格式化数据 用来导出csv
        const arr = res.referrers || [];
        formatArr(arr);
      });
    }
  }, []);
    ...
    >
      <div className={styles.box}>
        <div className={styles.btn}>
          ...
          /> */}
          <CSVLink data={csvData} filename="商务收益明细.csv" target="_blank">
        ...
        <div className={styles.checkStatement} ref={printRef}>
          <div className={styles.total_box}>
            <div className={styles.total_title}>
              ...
              <span style={{ fontSize: 14 }}>商务收益明细</span>
              <div>
          ...
          <div className={styles.data}>
            {data.referrers &&
              data.referrers.map((item: any, index: number) => {
                return (
                  <div className={styles.commodity_table} key={index}>
                    <ul className={styles.statistics}>
                      <li className={styles.mobile}>{item.referrerMobile}</li>
                      <li></li>
                      <li></li>
                    ...
                    <div className={styles.ul_box}>
                      {item.customers &&
                        item.customers.map((childItem: any, childIndex: number) => {
                          return (
                            <ul key={childIndex}>
                              <li></li>
                              <li>{childItem.customerMobile}</li>
